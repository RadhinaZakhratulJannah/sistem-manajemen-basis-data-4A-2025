...
CREATE VIEW view_umkm_detail AS SELECT 
    ...
    u.tahun_berdiri
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.nik
JOIN kategori_umkm k ON u.id_kategori = k.nama_kategori
JOIN skala_umkm s ON u.id_skala = s.nama_skala
JOIN kabupaten_kota kk ON u.id_kabupaten_kota = kk.nama_kabupaten_kota;
...
CREATE VIEW view_pemilik_dan_usaha AS SELECT 
    ...
    u.nama_usaha
FROM pemilik_umkm p
JOIN umkm u ON p.nik = u.id_pemilik;
...
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.nik
WHERE u.id_skala = 'Menengah';
SELECT * FROM view_umkm_menenga h;
...
LEFT JOIN umkm u ON u.id_kabupaten_kota = kk.nama_kabupaten_kota
